...
# 1. Usamos el enlace que tú sugeriste (que sí funciona)
URL_WEB = "https://rpp.pe/ultimas-noticias"
NOMBRE_ARCHIVO = "noticias_rpp_filtradas.csv"

# 2. Palabras clave para filtrar solo lo que nos interesa
PALABRAS_CLAVE = ["robo", "asalto", "delincuencia", "policía", "crimen", "sicario", "balacera", "muerte", "asesinato", "comisaría"]
...
def extraer_noticias_html():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"👀 Se leyeron {len(titulares)} titulares en total. Filtrando por seguridad...")
                ...
                for header in titulares:
                    ...
                    if enlace:
                        ...
                        if any(palabra in titulo_lower for palabra in PALABRAS_CLAVE):
                            writer.writerow([titulo_texto, url_noticia, "Seguridad/Policial"])
                            contador += 1
                            print(f"   -> Encontrado: {titulo_texto[:40]}...")
            
            if contador > 0:
                print(f"🎉 ¡Éxito! Se guardaron {contador} noticias relevantes en: {NOMBRE_ARCHIVO}")
            else:
                print("⚠️ Se accedió a la web, pero ninguna noticia de hoy contenía las palabras clave.")
                print("Prueba agregando más palabras al filtro o ejecutándolo más tarde.")
